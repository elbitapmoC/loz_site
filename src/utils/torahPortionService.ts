...
export class TorahPortionService {
  ...
  private findCurrentWeekPortionOptimized(
    ...
    while (left <= right) {
      ...
      // A Torah portion read on Saturday is current from that Saturday through the following Friday
      const saturdayDate = new Date(portion.date + 'T00:00:00');
      const saturdayStart = new Date(saturdayDate);
      saturdayStart.setHours(0, 0, 0, 0);
      
      const fridayEnd = new Date(saturdayDate);
      fridayEnd.setDate(saturdayDate.getDate() + 6);
      fridayEnd.setHours(23, 59, 59, 999);

      if (
        currentDate >= saturdayStart &&
        currentDate <= fridayEnd
      ) {
        return portion; // Found exact match
      } else if (currentDate < saturdayStart) {
        right = mid - 1;
      } else {
  ...
  // Get previous Torah portion
  getPreviousPortion(): TorahPortion | null {
    const currentPortion = this.getCurrentPortion();
    const currentIndex = this.portions.findIndex(
      (p) => p.key === currentPortion.key,
    );
    ...
    return this.portions[this.portions.length - 1]; // Cycle to end
  }
