...
export function ScriptureQuote({ 
  ...
  const variants = {
    ...
    overlay: "bg-white/10 backdrop-blur-sm border-white/20 text-white/95",
  ...
  if (variant === "overlay") {
    return (
      <motion.div
        ...
        `}
      >
        <p className="italic text-lg text-white/95 mb-2">
          "{quote}"
        </p>
        <cite className="text-sm text-white/70">— {reference}</cite>
      </motion.div>
    );
  }
